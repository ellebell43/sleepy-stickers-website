...
export function getCartTotalPrice(otherCart?: cartItem[]): number {
  let cart = otherCart ? otherCart : getCartArray()
  let price = 0
  cart.map((el) => { price += ((Number(el.productType.price) + el.size.price) * el.quantity) })
  return price
}
...
export function findItemIndex(productID: string, typeID: string, sizeID: string, otherCart?: cartItem[]): number | undefined {
  let cart = otherCart ? otherCart : getCartArray()
  let index: number | undefined = undefined
  cart.map((el: cartItem, i: number) => {
    if (el.product.id == productID && el.productType.id == typeID && el.size.id == sizeID) {
      index = i
    }
  })
  return index
